...
public class ScenarioBorrowBook 
{
	...
	@Test
	public void testBorrowingEnabled()
	{		
	...
	@Test
	public void testBorrowingRestrictedWithFines()
	{
		testControlClass_.setState(EBorrowState.INITIALIZED);
		testControlClass_.cardSwiped(3);

		verify(testCardReader_).setEnabled(false);
		verify(testUI_).setState(EBorrowState.BORROWING_RESTRICTED);
		verify(testUI_).displayMemberDetails(3, "firstName2 lastName2", "0003");
		verify(testUI_).displayOutstandingFineMessage(10.0f);
		verify(testUI_).displayOverFineLimitMessage(10.0f);  

		assertEquals(EBorrowState.BORROWING_RESTRICTED, testControlClass_.getState());
	}

	@Test
	public void testBorrowingRestrictedWithOverLimit()
	{
		testControlClass_.setState(EBorrowState.INITIALIZED);
		testControlClass_.cardSwiped(4);

		verify(testCardReader_).setEnabled(false);
		verify(testUI_).setState(EBorrowState.BORROWING_RESTRICTED);
		verify(testUI_).displayMemberDetails(4, "firstName3 lastName3", "0004");
		verify(testUI_).displayAtLoanLimitMessage();
		verify(testUI_).displayExistingLoan(any(String.class));
	...
	@Test
	public void testBorrowingRestrictedOverDueLoan()
	{
		testControlClass_.setState(EBorrowState.INITIALIZED);
		testControlClass_.cardSwiped(2);

		verify(testCardReader_).setEnabled(false);
		verify(testUI_).setState(EBorrowState.BORROWING_RESTRICTED);
		verify(testUI_).displayMemberDetails(2, "fName1 lName1", "0002");
		verify(testUI_).displayOverDueMessage();
		verify(testUI_).displayExistingLoan(any(String.class));
	...
	@Test
	public void testBorrowingMemberDoesNotExist()
	{
		testControlClass_.setState(EBorrowState.INITIALIZED);
		testControlClass_.cardSwiped(7);

		verify(testCardReader_).setEnabled(true);
		verify(testScanner_).setEnabled(false);
		verify(testUI_).setState(EBorrowState.INITIALIZED);
		verify(testUI_).displayErrorMessage(any(String.class));

		assertEquals(EBorrowState.INITIALIZED, testControlClass_.getState());
	}	
	
	@Test(expected=RuntimeException.class)
	public void testCardSwipedIsInWrongState() 
	{
		testControlClass_.cardSwiped(1);
	}
}
